...
export function Layout() {
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `px-3 py-2 text-xs sm:text-base rounded-xl ${isActive ? "bg-black text-white" : "hover:bg-black/10"}`;
    ...
    <div className="min-h-screen">
      <header className="border-b">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between gap-3">
          <div className="font-semibold text-xs sm:text-base">{t("app_name")}</div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center rounded-xl border p-1 text-xs">
              ...
              <button
                type="button"
                onClick={() => setLang("en")}
                className={`px-2 text-xs sm:text-base py-1 rounded-lg ${lang === "en" ? "bg-black text-white" : "hover:bg-black/10"}`}
